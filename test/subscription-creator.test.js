...
// nct is 1 throughout. These cases are about creator, not about notification content, and
// "Modified Attributes" is n/a for net=3 in TS-0001:9.6.8 table 9.6.8-4 -- the pair used to be
// accepted and ignored, so it sat here unnoticed until the table was enforced.
...
test("creator is set automatically when notificationURI is not the Originator", async () => {
  ...
  const res = await create(srv.baseUrl, cnt, 23, {
    "m2m:sub": { rn: "s", nu: [sink.url], nct: 1, enc: { net: [3] } },
  });
  assert.equal(res.rsc, "2001");
...
test("an explicit null still means 'fill it in for me'", async () => {
  ...
  const res = await create(srv.baseUrl, cnt, 23, {
    "m2m:sub": { rn: "s", nu: [sink.url], nct: 1, cr: null, enc: { net: [3] } },
  });
  assert.equal(res.rsc, "2001");
...
test("creator is left unset when every notificationURI is the Originator itself", async () => {
  ...
  const res = await create(srv.baseUrl, cnt, 23, {
    "m2m:sub": { rn: "s", nu: [ADMIN], nct: 1, enc: { net: [3] } },
  });
  assert.equal(res.rsc, "2001");
...
test("a subscription cannot name someone else as its creator", async () => {
  ...
  const res = await create(srv.baseUrl, cnt, 23, {
    "m2m:sub": { rn: "s", nu: [sink.url], nct: 1, cr: "SomeOtherEntity", enc: { net: [3] } },
  });
...
test("naming yourself as creator is accepted as the no-op it is", async () => {
  ...
  const res = await create(srv.baseUrl, cnt, 23, {
    "m2m:sub": { rn: "s", nu: [sink.url], nct: 1, cr: ADMIN, enc: { net: [3] } },
  });
  assert.equal(res.rsc, "2001");
