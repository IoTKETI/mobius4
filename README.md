...
| 4.19.0 | 2026-09-01 | A `<subscription>` can now name **which attributes** should wake it: `enc.atr` (the `attribute` condition of `eventNotificationCriteria`). `net=1` then fires only when the UPDATE touches one of the named attributes, instead of on every attribute update. Carrying `atr` used to be refused `4000`. Subscriptions that do not set it behave exactly as before | [nothing required](docs/upgrading.md#v4190) |
