...
| Unreleased | — | **The administrator bypasses access control again.** `cse.admin` is granted every operation before any `<accessControlPolicy>` is read, reversing the v4.6.0 change below: a resource created with no `accessControlPolicyIDs` is governed by its creator, which left the administrator unable to reach it *and* unable to attach a policy to it — no request could undo the state. Deliberately non-conformant; `cse.admin` is now a full bypass and must be treated as a credential | [nothing required](docs/upgrading.md#unreleased) |
