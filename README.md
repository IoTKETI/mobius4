...
| 4.24.5 | 2026-09-07 | **A notification sent to an `<AE>` by resource ID carried an empty `To`** — it went to the bare `pointOfAccess`, which is where to send it, not what it addresses. Affects subscription verification too. **Outgoing notifications are now readable in the log**, and one that cannot be delivered at all no longer passes in silence | [**read if a `notificationURI` names a resource**](docs/upgrading.md#v4245) |