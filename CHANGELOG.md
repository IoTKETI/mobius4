...
아래는 이번 작업 범위 밖이지만 검증 과정에서 확인된 것들이다. 심각한 순서로 적는다.

- **잘못된 `gmty` 값이 디스커버리의 범위 제한을 통째로 무력화한다.** `set_where_clause`가
  지오쿼리 분기 두 곳에서 `return where;`로 빠져나가는데, 호출부는
  `{ where, has_geo_query }`로 구조분해한다 → `where`가 `undefined`가 되어
  `findAll({ where: undefined })`가 되고 **대상 서브트리 밖 리소스까지 타입 테이블 전체가
  반환**된다. `prim_schema.js`의 `gmty`에는 범위 검증이 없어(`gsf`는 1..3으로 제한됨)
  `?fu=1&gmty=9&gsf=1&geom=[1,2]` 같은 요청으로 도달할 수 있다. 리소스별 접근제어는 여전히
  돌지만 주소 지정한 범위를 벗어난다.
- **디스커버리 SQL 오류가 조용히 성공으로 둔갑한다.** `cse/reqPrim.js`의 `fu1_discovery`
  호출이 예외를 삼키고 로그만 남긴다. SQL이 실패해도 5000이 아니라 **빈 목록 + RSC 2000**이
  된다. `lvl` 구현 중 잘못된 `where.lvl`이 에러가 아니라 빈 결과로 나타나 진단이 늦어졌다.
- **디스커버리 LIKE 패턴의 와일드카드가 이스케이프되지 않는다.** 대상 sid에 `_`나 `%`가
  들어 있으면 형제 리소스까지 과다 매칭된다(`lvl` 결과도 함께 어긋난다).
